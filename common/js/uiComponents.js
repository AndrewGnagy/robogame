// DEPRECATING THE COMMENTED CODE BELOW
...
robo.namespace("dialogGarage")
robo.dialogGarage.isUp = false
robo.dialogGarage.show = function(inputText,name,callback) {
	if(typeof inputText === "string")
		inputText = [inputText];
	if(!name)
		name = "";
	if(!robo.dialogGarage.isUp){
		robo.dialogGarage.callback = callback;
		robo.dialogGarage.textAry = inputText;
		robo.dialogGarage.name = name;
		robo.dialogGarage.isUp = true;
		robo.dialogGarage.advance();
		$('#dialogGarage').show();
	}
}

robo.dialogGarage.advance = function(){
	$('#dialogGarage .panel-heading').empty();
	$('#dialogGarage .panel-body').empty();
	var txt = robo.dialogGarage.textAry.shift();
	if(txt){
		$('<p>'+robo.dialogGarage.name+'</p>').appendTo('#dialogGarage .panel-heading');
		$('<p>'+txt+'</p>').appendTo('#dialogGarage .panel-body');
	} else {
		if(robo.dialogGarage.callback)
			robo.dialogGarage.callback();
		robo.dialogGarage.callback = undefined;
		robo.dialogGarage.hide();
	}
	console.log(txt);
};

robo.dialogGarage.hide = function(){
	robo.dialogGarage.isUp = false;
	$('#dialogGarage').hide();
};
